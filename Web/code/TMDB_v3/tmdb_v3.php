...
//end of movieDetail
    ...
    public function moviePoster($idMovie) {
        ...
        //if no posters were found, try again WITHOUT the language
        if (count($posters) == 0) {
            $lang = $this->getLang();
            $this->setLang(null);
            $posters = $this->movieInfo($idMovie, "images", false);
...
//end of getConfig
    ...
    private function _call($action, $text, $lang = "") {
        ...
        $langStr = "";
        //if the language is null, then the user specified that they did NOT want to use a language at all
        if ($lang != null) {
            $langStr = "&language=$lang";
        }
        $url = TMDBv3::_API_URL_ . $action . "?api_key=" . $this->getApikey() . "$langStr&" . $text;
        // echo "<pre>$url</pre>";
        $ch = curl_init();
